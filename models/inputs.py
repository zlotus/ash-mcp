...
class ValueCandidatesGridInput(BaseModel):
    model_config = ConfigDict(str_strip_whitespace=True)
    anchor_symbol: str = Field(default="601318", description="Anchor stock, e.g. 601318 (Ping An)")
    ...
    candidate_limit: int = Field(default=80, ge=20, le=300, description="Max candidate stocks to evaluate")
    ...
    strict_same_industry: bool = Field(default=False, description="Only keep candidates in exactly the same industry as anchor")
    max_pe_percentile: float = Field(default=45.0, ge=1, le=100, description="Max allowed PE percentile")
    max_pb_percentile: float = Field(default=50.0, ge=1, le=100, description="Max allowed PB percentile")
    min_roe: float = Field(default=8.0, ge=-50, le=80, description="Minimum ROE(%)")
    max_debt_ratio: float = Field(default=92.0, ge=0, le=100, description="Maximum debt ratio(%)")
    min_yoy_net_profit: float = Field(default=-15.0, ge=-300, le=500, description="Minimum YoY net profit growth(%)")
    min_dividend_years_5y: int = Field(default=2, ge=0, le=5, description="Minimum dividend-paying years in last 5 years")
    market_cap_ratio_min: float = Field(default=0.3, ge=0.0, le=10.0, description="Min market-cap ratio vs anchor")
    market_cap_ratio_max: float = Field(default=3.0, ge=0.1, le=20.0, description="Max market-cap ratio vs anchor")
    max_annual_volatility: float = Field(default=0.55, ge=0.05, le=2.0, description="Max annualized volatility")
